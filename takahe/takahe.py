...
class word_graph:
    ...
    def __init__(self,
        ...
        self.verbs = set([
            'VB', 'VBD', 'VBP', 'VBZ', 'VH', 'VHD', 'VHP', 'VBZ', 'VV', 'VVD',
            'VVP', 'VVZ'
        ])
        """
        ...
        for i in range(self.length):
            ...
            for j in range(sentence_len):
                ...
                if mapping[j] == 0:
                    ...
                    while not found:
                        ...
                        # Get the sentences id of this node
                        ids = []
                        for sid, p in self.graph.node[(node,
                                                       selected)]['info']:
                            ids.append(sid)
                    ...
                    # Update the node in the graph if not same sentence
                    if found:
                        self.graph.node[(node, selected)]['info'].append((i,
                                                                          j))
            ...
            for j in range(sentence_len):
                ...
                # Else find the node with overlap in context or create one
                    ...
                    ids = []
                    for sid, pos_s in self.graph.node[(node,
                                                       selected)]['info']:
                        ids.append(sid)
    ...
    def get_directed_context(self, node, k, dir='all', non_pos=False):
        ...
        for sid, off in self.graph.node[(node, k)]['info']:
            ...
            prev = self.sentence[sid][off - 1][0].lower(
            ) + self.sep + self.sentence[sid][off - 1][1]

            next = self.sentence[sid][off + 1][0].lower(
            ) + self.sep + self.sentence[sid][off + 1][1]
        ...
        while len(kshortestpaths) < k and len(orderedX) > 0:
            ...
            for node in self.graph.neighbors(shortest[1]):
                ...
                if node == end:
                    ...
                    if nb_verbs > 0 and length >= self.nb_words and paired_parentheses == 0 and (
                            quotation_mark_number %
                            2) == 0 and raw_sentence not in sentence_container:
                        path = [node]
    ...
    def get_compression(self, nb_candidates=50):
        ...
        self.paths = self.k_shortest_paths(
            ...
            (self.stop + self.sep + self.stop, 0), nb_candidates)
...
class keyphrase_reranker:
    ...
    def score_keyphrase_candidates(self):
        ...
        for keyphrase in self.keyphrase_candidates:
            ...
            keyphrase_score /= (
                len(self.keyphrase_candidates[keyphrase]) + 1.0)
